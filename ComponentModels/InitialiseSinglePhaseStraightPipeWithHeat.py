from Algorithms.DesignToolAlgorithmV2.ComponentModels.SinglePhaseStraightPipeCellWithHeat import SinglePhaseStraightPipeCellWithHeat
...
class SinglePhaseStraightPipeWithHeat():
    def __init__(self, Geometry, prop1, prop2, propsGiven, vel_x, Eilmer_fluid_file, \
        ...
        self.initialiseCells(Geometry = Geometry, prop1 = prop1, \
                                ...
                                fluidPairs = fluidPairs, heatFlux = heatFlux)
    ...
    def initialiseCells(self, Geometry, prop1, prop2, propsGiven, vel_x, componentLabel, fluidPairs, heatFlux):
        [D, L, nCells] = Geometry
        self.nCells = nCells
        
        for cell in range(nCells):
            cellObject = SinglePhaseStraightPipeCellWithHeat(cell_ID = cell, fluidPair = fluidPairs, label = componentLabel, heatFlux = heatFlux)
            GEO = {
                "dx"    :   L / nCells,
